...
3. Concatenate the transformed output in that same order into one string. This is behaviorally equivalent to today's sequential `<script>` execution: classic (non-module) scripts in one document share a single global lexical environment, so top-level `const`/`let`/`function` declarations in file A are already visible to file B today, exactly as concatenation would preserve.
...
- This is the first build step ever added to this folder — if the concatenation-order assumption is wrong for some file (e.g., an as-yet-undiscovered reliance on script-tag isolation rather than shared scope), it would surface immediately in local verification (step 1-3 above) before any deploy.
