...
const SCREENSHOTS_DIR = '/home/gabriel/docker/code-server/config/workspace/LeadAwakerApp/test-results/feat-9-10-11-v2';
 ...
 *
 * From the source code (RightSidebar.tsx lines 576-589):
 ...
 *   </div>
 *
 * We find it by looking for the button that contains the text "Collapse" (when expanded)
 * or the PanelRightClose icon (when collapsed).
 */
async function clickCollapseToggle(page) {
  ...
  // When collapsed, the button only has an icon - it's the first button in the bottom section
  // The bottom actions div has class "px-3 mb-1 space-y-1 shrink-0"
  // We use JS to find it: button inside aside that does NOT have data-testid
  // and is the first button in the bottom section
  const clicked = await page.evaluate(() => {
    const aside = document.querySelector('aside[data-sidebar-focus]');
    if (!aside) return false;
    // The bottom actions div is .px-3.mb-1.space-y-1.shrink-0
...
async function main() {
  ...
  if (initialWidth < 100) {
    // Something is wrong - sidebar is already collapsed, reset and reload
  ...
  // Expected nav items from source: home, accounts, campaigns, contacts, chats, calendar, tags, library, users, automations, settings = 11
  ...
  // From source: agencyOnly items: accounts, tags, library, users, automations, settings
  // Viewer should only see: home, campaigns, contacts, chats, calendar = 5 items
  ...
  // As Viewer, try to access an agency-only page
  ...
  R.push({ id: 'F10-S3', label: `Restricted URL redirected (landed at: ${urlAfterRestricted.replace('http://localhost:5173', '')})`, pass: f10s3Pass });
  ...
  // Switcher is inside [data-testid="sidebar-account-switcher"] which is visible for isAgencyUser
  ...
  if (switcherVisible) {
    ...
    if (optCount > 1) {
      ...
      // Click a different account (not the first/current one)
