...
const SCREENSHOT_DIR = path.join(__dirname, '..', 'test-results', 'feat-134-dnd');
...
async function main() {
  ...
  await page.goto('http://localhost:5173/agency/calendar', { waitUntil: 'networkidle' });
  ...
  await shot(page, '01-calendar-month');
  ...
  await shot(page, '03-booking-card');
