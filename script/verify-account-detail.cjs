...
const SCREENSHOT_DIR = path.join(__dirname, '..', 'test-results', 'account-detail-verify');
...
async function main() {
  ...
  console.log('\n=== STEP 3: Click on first account row ===');
  // Rows have id="row-{Id}" — find the first one
  const firstRow = await page.$('tr[id^="row-"]');
  if (!firstRow) {
    console.log('FAIL: No account rows found in table');
