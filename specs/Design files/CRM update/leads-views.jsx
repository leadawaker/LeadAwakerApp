...
function LeadsApp() {
  ...
  // Collapse the rail to icon-only below 1240px; collapse Filter/Sort/Group → ⚙ below 1200px
  const railCollapsed   = vw < 1240;
  const toolbarCollapsed = vw < 1200;
  ...
  return (
    <div className="la-app" style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', overflow: 'hidden', background: 'var(--bg)' }}>
        ...
        {/* ── Left rail card: owns the full page header ── */}
        <div style={{
          width: railCollapsed ? 68 : 356,
          flexShrink: 0,
          display: 'flex',
          ...
          {railCollapsed ? (
            /* Narrow: just an icon marker in the 60px header slot */
            <div style={{ height: 60, flexShrink: 0, borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <>
              ...
              <div style={{ height: 60, flexShrink: 0, padding: '0 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid var(--line)' }}>
                {selCount > 0 ? (
                  <>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--wine)', fontWeight: 700 }}>{selCount} selected</span>
                    <div style={{ flex: 1 }} />
                    {['Bump', 'Tag', 'Export'].map(a => (
                      <button key={a} className="la-btn la-btn--soft" style={{ fontSize: 10, padding: '5px 9px' }}>{a}</button>
                    ))}
                    <button onClick={() => setSelected(new Set())} className="la-btn la-btn--soft la-btn--icon"><IconX size={12} /></button>
                  </>
                ) : (
                  <>
                    <span style={{ fontFamily: 'var(--serif)', fontSize: 22, color: 'var(--ink)', letterSpacing: '-0.01em' }}>Leads</span>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--mute-2)', letterSpacing: '0.1em' }}>#{D.leads.length}</span>
                  </>
                )}
              </div>

              {/* View switcher — taller padding, 11px, tracked caps */}
              <div style={{ padding: '12px 16px 0', flexShrink: 0 }}>
                <div className="la-seg la-seg--fill" style={{ width: '100%' }}>
                  {views.map(({ key, Ic, label }) => (
                    <button key={key} onClick={() => setView(key)}
                      className={`la-seg-btn${view === key ? ' on' : ''}`}
              ...
              {/* Toolbar: search + Filter/Sort/Group (or ⚙) + wine + */}
              <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                <div style={{ position: 'relative', flex: '1 1 80px', minWidth: 0 }}>
                  <input className="neu-input" placeholder="Search leads…" style={{ padding: '7px 10px 7px 27px', fontSize: 11, width: '100%' }} />
                ...
                <button title="New lead" className="la-btn la-btn--wine la-btn--icon"><IconPlus size={14} /></button>
              </div>

              {/* Rule */}
              <div style={{ height: 1, background: 'var(--line)', margin: '0 16px 10px', flexShrink: 0 }} />
            </>
          )}
        ...
        {/* ── Right content area: detail / table / kanban ── */}
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {view === 'list' ? (
            detail ? (
              <LeadDetail detail={detail} clientView={clientView} onToggleClient={() => setClientView(v => !v)} />
            ) : (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--mute-2)', fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                Select a lead
              </div>
            )
          ) : view === 'table' ? (
            <TableView leads={D.leads} activeId={activeId} selected={selected}
              onSelectLead={(id) => setActiveId(id === activeId ? null : id)}
              onToggle={toggle} onToggleAll={toggleAll} allSelected={allSelected} />
          ) : (
            <KanbanView leads={D.leads} activeId={activeId}
              onSelectLead={(id) => setActiveId(id === activeId ? null : id)} />
          )}
        </div>
