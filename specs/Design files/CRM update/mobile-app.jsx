...
function MobMore({ onNavigate }) {
  ...
  const routable = { Billing: 'Billing' };
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
...
function MobileApp() {
  ...
  return (
    <div className="la-app" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* Billing is a sub-screen reached from More — keep More lit while there */}
      <MobBottomNav active={tab === 'Billing' ? 'More' : tab} onTab={setTab} />
    </div>
  );
}
