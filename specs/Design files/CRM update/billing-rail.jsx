...
// ═══ The rail — owns the full left-card header: title + tabs + toolbar ═══
function BillingRail({ tab, setTab, tabOpts,
  ...
  return (
    <div style={{ width: 356, flexShrink: 0, background: 'var(--bg)', borderRight: '1px solid var(--line)', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>

      {/* 60px title header row */}
      <div style={{ height: 60, flexShrink: 0, borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', padding: '0 14px', gap: 8 }}>
        <span style={{ fontFamily: 'var(--serif)', fontSize: 22, color: 'var(--ink)', letterSpacing: '-0.01em', flex: 1 }}>Billing</span>
        <BSeg value={view} onChange={setView} options={[{ key: 'list', label: '', Ic: BIList }, { key: 'table', label: '', Ic: BITable }]} />
      </div>

      {/* Invoices / Expenses / Contracts seg — taller tabs */}
      <div style={{ padding: '12px 10px 0' }}>
        <div className="la-seg la-seg--fill">
          {tabOpts.map(o => {
            const on = o.key === tab;
            return (
              <button key={o.key} onClick={() => setTab(o.key)} className={`la-seg-btn${on ? ' on' : ''}`}
                style={{ padding: '10px 14px', fontSize: 11, letterSpacing: '0.13em', display: 'flex', alignItems: 'center', gap: 6 }}>
                <o.Ic size={13} />{o.label}
              </button>
            );
          })}
        </div>
      ...
      {/* Toolbar: search + Filter toggle + New */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '10px 10px 0' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
          <input className="neu-input" placeholder="Search…" style={{ padding: '7px 10px 7px 30px', width: '100%', fontSize: 12, boxSizing: 'border-box' }} />
          <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: 'var(--mute-2)', display: 'flex' }}><BISearch size={13} /></span>
        </div>
        <button title="Filter" onClick={() => setShowFilter(s => !s)} className="la-btn la-btn--soft la-btn--icon"
          style={showFilter ? { color: 'var(--wine)', boxShadow: 'var(--sh-inset-crisp)' } : undefined}>
          <BIFilter size={14} />
        </button>
        {!clientView && (
          <button onClick={onNew} title={newLabel} className="la-btn la-btn--wine la-btn--icon">
            <BIPlus size={16} />
          </button>
        )}
      </div>
