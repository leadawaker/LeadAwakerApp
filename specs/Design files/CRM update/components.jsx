...
function CampaignList({ width = 356, tab, setTab }) {
  ...
  const hasTabs  = tab !== undefined && setTab !== undefined;

  // Collapse filter/sort/group into a single ⚙ button on narrower viewports
  ...
  const segTabs = [
    { key: 'stats',    Ic: IconActivity, label: 'Stats' },
    { key: 'settings', Ic: IconSettings, label: 'Settings' },
  ];
  ...
  return (
    <div style={{
      ...
      {/* ── Title header — height matches sidebar logo row ── */}
      <div style={{ height: 60, flexShrink: 0, padding: '0 4px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid var(--line)' }}>
        <span style={{ fontFamily: 'var(--serif)', fontSize: 22, color: 'var(--ink)', letterSpacing: '-0.01em' }}>Campaigns</span>
        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--mute-2)', letterSpacing: '0.1em' }}>#{D.active.id}</span>
      </div>

      {/* ── Stats / Settings seg ── */}
      {hasTabs && (
        <div style={{ padding: '12px 4px 0', flexShrink: 0 }}>
          <div className="la-seg la-seg--fill" style={{ width: '100%' }}>
            {segTabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`la-seg-btn${tab === t.key ? ' on' : ''}`}
      ...
      {/* ── Toolbar: search + controls + new ── */}
      <div style={{ padding: '10px 4px 10px', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
        <div style={{ position: 'relative', flex: '1 1 80px', minWidth: 0 }}>
          <input className="neu-input" placeholder="Search…" style={{
            padding: '7px 10px 7px 27px', fontSize: 11, width: '100%',
          }} />
      ...
      <div className="rule" style={{ margin: '0 4px 10px', flexShrink: 0 }} />
