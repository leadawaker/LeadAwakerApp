...
Cover: overall performance highlights, what's working well, pipeline bottlenecks (where are leads stalling?), and one actionable recommendation.`;
  ...
  // ─── Automation Health ──────────────────────────────────────────────
  // Agency-only: merged engine scheduler state + Postgres last-run per job

  const JOB_GRACE_MAP: Record<string, number> = {
    campaign_launcher:      60 * 2,        // 2× 60s cadence
    bump_scheduler:         60 * 5 * 2,    // 2× 5m
    demo_bump_scheduler:    60 * 5 * 2,
    lead_scorer:            60 * 30 * 2,   // 2× 30m
    task_reminders:         60 * 15 * 2,   // 2× 15m
    buying_signal_followup: 60 * 5 * 2,
    metrics_aggregator:     60 * 60 * 26,  // 26h for daily
  ...
  const HEALTH_CACHE_TTL = 25_000; // 25s
  ...
  app.get("/api/automation-health", requireAgency, wrapAsync(async (_req, res) => {
    ...
    const jobs = KNOWN_JOBS.map(id => {
      ...
      if (!engineHealthy || !eng) {
        ...
        // never run — treat as overdue only if engine has been up long enough
        status = "overdue";
      }
