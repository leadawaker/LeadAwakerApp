...
function FilterTabs({ value, setValue, counts }) {
  ...
  return (
    <div style={{ height: HEADER_H, flexShrink: 0, padding: '0 12px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 4 }}>
      {tabs.map(([k, label]) => {
        const on = k === value;
        return (
...
function CenterHeader({ rangeLabel, onPrev, onNext, onToday }) {
  return (
...
function CalendarApp() {
  ...
  // Collapse the agenda to an icon rail + stack the detail facts when the
  // content area is tight, so the calendar keeps a roomy near-square shape
  // (same behaviour as the Leads list).
  ...
  return (
    <div className="la-app" style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)' }}>
        <CalendarHeader view={view} setView={setView} stats={C.stats} />
        {/* 3 white floating cards, aligned headers, fills height */}
        <div style={{ flex: 1, minHeight: 0, display: 'flex', gap: 14, padding: '4px 14px 14px', overflow: 'hidden' }}>

          {/* LEFT — agenda (cream rail, like Campaigns; white selected card) */}
          <div style={{ ...cardStyle, width: narrow ? 68 : 318, flexShrink: 0, background: 'var(--bg-2)' }}>
            {narrow ? (
              <div style={{ height: HEADER_H, flexShrink: 0, borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ width: 34, height: 34, borderRadius: 'var(--r-button)', background: 'var(--surface)', boxShadow: 'var(--sh-raised-crisp)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--wine)' }}><CIconClock size={15} /></span>
              </div>
            ) : (
          ...
          {/* CENTER — calendar (cream surface, white event cards) */}
          <div style={{ ...cardStyle, flex: 1, minWidth: 0, background: 'var(--bg-2)' }}>
            <CenterHeader rangeLabel={rangeLabel} onPrev={prev} onNext={next} onToday={today} />
            {view === 'week'
