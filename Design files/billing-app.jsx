    ...
    <div className="la-app" style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)', position: 'relative' }}>
        {formOpen ? (
          ...
          <>
            {/* ── Top header: title + tabs + view toggle + toolbar ── */}
            <div style={{ height: 60, flexShrink: 0, padding: '0 20px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 16 }}>
              <span style={{ fontFamily: 'var(--serif)', fontSize: 24, color: 'var(--ink)', letterSpacing: '-0.01em' }}>Billing</span>
              <div className="la-seg">
                {tabOpts.map(o => (
                  <button key={o.key} onClick={() => setTab(o.key)} className={`la-seg-btn${o.key === tab ? ' on' : ''}`}>
                    <o.Ic size={14} />{o.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <span style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: 'var(--mute-2)', display: 'flex', pointerEvents: 'none' }}><BISearch size={13} /></span>
                </div>
                <button title="Filter" className="la-btn la-btn--soft la-btn--icon"><BIFilter size={14} /></button>
                {!clientView && <button onClick={() => setFormOpen(true)} title={newLabel} className="la-btn la-btn--wine"><BIPlus size={14} />{newLabel}</button>}
              </div>
            </div>
            {/* ── Content ── */}
