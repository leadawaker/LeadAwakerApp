...
function TasksLeftPanel({ view, setView, filter, setFilter, who, setWho, counts, tasks, activeId, onSelect, onToggle }) {
  ...
  return (
    <div style={{ width: 300, flexShrink: 0, borderRight: '1px solid var(--line)', background: 'var(--bg-2)', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
  ...
  return (
    <div className="la-app" style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg)' }}>
        {/* Top header: title + view seg + toolbar */}
        <div style={{ height: 60, flexShrink: 0, padding: '0 20px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--mute-2)', letterSpacing: '0.1em' }}>{allTasks.length}</span>
          <div className="la-seg">
            {[{ key: 'schedule', Ic: TIconSplit, label: 'Schedule' }, { key: 'board', Ic: TIconBoard, label: 'Board' }].map(v => (
              <button key={v.key} onClick={() => setView(v.key)} className={`la-seg-btn${view === v.key ? ' on' : ''}`}>
                <v.Ic size={14} />{v.label}
              </button>
            ))}
          </div>
          <div style={{ flex: 1 }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ position: 'relative' }}>
              <input className="neu-input" placeholder="Search tasks…" style={{ paddingLeft: 32, fontSize: 12, width: 190, padding: '8px 12px 8px 32px' }} />
              <span style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: 'var(--mute-2)', display: 'flex', pointerEvents: 'none' }}><TIconSearch size={13} /></span>
            </div>
            ...
            <button title="Add task" className="la-btn la-btn--wine"><TIconPlus size={13} />Add task</button>
          </div>
        </div>
        {/* Content area */}
