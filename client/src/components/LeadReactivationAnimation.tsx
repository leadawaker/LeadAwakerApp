...
const LeadReactivationAnimation = () => {
  ...
  return (
    <div className="relative w-full h-[400px] bg-slate-50 flex items-center justify-center overflow-hidden font-sans rounded-xl border border-border">
      <style>{`
        @keyframes sparkle {
...
const Cursor = ({ startX, startY, onHover }: CursorProps) => {
  ...
  const path = useMemo(() => {
    // Generate a slightly deviated path
    const midX1 = startX + (50 - startX) * 0.3 + (Math.random() * 12 - 6);
    const midY1 = startY + (50 - startY) * 0.3 + (Math.random() * 12 - 6);
    
    const midX2 = startX + (50 - startX) * 0.7 + (Math.random() * 8 - 4);
    const midY2 = startY + (50 - startY) * 0.7 + (Math.random() * 8 - 4);
