...
const NodeStrokeAnimation = () => (
  <style>{`
    ...
    .card-active-amber .card-gradient-overlay {
      background: linear-gradient(to top left, rgba(245, 158, 11, 0.12), transparent 70%);
    }
    .card-active-purple .card-gradient-overlay {
      background: linear-gradient(to top left, rgba(168, 85, 247, 0.12), transparent 70%);
    }
    .card-active-emerald .card-gradient-overlay {
      background: linear-gradient(to top left, rgba(16, 185, 129, 0.12), transparent 70%);
    }
    .card-active-cyan .card-gradient-overlay {
      background: linear-gradient(to top left, rgba(6, 182, 212, 0.12), transparent 70%);
    }
  `}</style>
);
...
export default function WorkflowVisualization() {
  ...
  const getStatusColor = (node: string, activeClass: string, inactiveClass: string) => {
    return activeNode === node ? activeClass : inactiveClass;
  };
    ...
    <div ref={containerRef} className="flex flex-col items-center justify-center p-8 font-sans text-gray-900 md:overflow-hidden relative">
      ...
      <div className="relative w-full max-w-full md:h-[400px] bg-gradient-to-br from-gray-100 to-gray-200 border border-gray-300/50 rounded-3xl overflow-hidden shadow-lg flex flex-col md:flex-row items-center justify-between px-12 py-12 md:py-0 space-y-24 md:space-y-0">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 1 }}>
          <g className="hidden md:block">
            <line 
              ...
              className={`${isGlowing1 ? 'line-quick-glow' : ''} ${isFadingBack1 ? 'line-fade-back' : ''}`}
            />
            <line 
              x1="50%" y1="50%" x2="75%" y2="50%" 
              stroke="#d1d5db" strokeWidth="2"
              className={`${isGlowing2 ? 'line-quick-glow' : ''} ${isFadingBack2 ? 'line-fade-back' : ''}`}
            />
          </g>
        </svg>
        {/* Card: Contact */}
        <div className="relative z-10">
          ...
          <div className={`relative bg-white border-2 rounded-xl p-6 w-64 transition-all duration-300 ${getStatusColor('contact', 'border-amber-500 shadow-[0_20px_50px_-12px_rgba(245,158,11,0.6)] card-active-amber', 'border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.2)]')}`}>
            <div className="card-gradient-overlay" />
            <div className="flex items-center gap-3 mb-4">
              <div className={`p-2 rounded-lg ${activeNode === 'contact' ? 'bg-amber-100' : 'bg-gray-100'}`}>
                <User className={`w-5 h-5 ${activeNode === 'contact' ? 'text-amber-600' : 'text-gray-400'}`} />
              </div>
              <div>
                <h3 className={`font-bold text-sm transition-colors duration-300 ${activeNode === 'contact' ? 'text-gray-900' : 'text-gray-500'}`}>Lead #315</h3>
                <p className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">Jack Johnson</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-2 border border-gray-300 font-mono text-[10px] text-gray-700">
               {guardrailsStatus === 'Message Sent' ? 'Lead Engaged' : 'Not Engaged'}
            </div>
          </div>
        </div>
        {/* Card: AI Agent */}
        <div className="relative z-10">
          ...
          <div className={`relative bg-white border-2 rounded-xl p-6 w-64 transition-all duration-300 ${getStatusColor('agent', 'border-purple-500 shadow-[0_20px_50px_-12px_rgba(168,85,247,0.6)] card-active-purple', 'border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.2)]')}`}>
            <div className="card-gradient-overlay" />
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="bg-gray-50 rounded-lg p-2 border border-gray-300 h-[34px] flex items-center overflow-hidden">
                <span className="text-[10px] text-gray-700 font-mono truncate">
                  {activeNode === 'agent' ? (agentStatus === 'msg1' ? 'Good morning :)' : 'Is this Jack?') : '...'}
                </span>
            </div>
          </div>
        </div>
        {/* Card: Guardrails */}
        <div className="relative z-10">
          <div className={`absolute inset-0 rounded-xl blur-3xl transition-opacity duration-500 ${activeNode === 'guardrails' ? (isApproved ? 'bg-emerald-400/40' : 'bg-cyan-400/40') : 'opacity-0'}`} />
          <div className={`relative bg-white border-2 rounded-xl p-6 w-64 transition-all duration-300 ${isApproved ? 'border-emerald-500 shadow-[0_20px_50px_-12px_rgba(16,185,129,0.6)] card-active-emerald' : getStatusColor('guardrails', 'border-cyan-500 shadow-[0_20px_50px_-12px_rgba(6,182,212,0.6)] card-active-cyan', 'border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.2)]')}`}>
            <div className="card-gradient-overlay" />
            <div className="flex items-center gap-3 mb-4">
              <div className={`p-2 rounded-lg ${isApproved ? 'bg-emerald-100' : activeNode === 'guardrails' ? 'bg-cyan-100' : 'bg-gray-100'}`}>
                {isApproved ? <CheckCircle2 className="w-5 h-5 text-emerald-600" /> : <Shield className={`w-5 h-5 ${activeNode === 'guardrails' ? 'text-cyan-600' : 'text-gray-400'}`} />}
              </div>
              <div>
                <h3 className={`font-bold text-sm transition-colors duration-300 ${activeNode === 'guardrails' ? 'text-gray-900' : 'text-gray-500'}`}>Guardrails</h3>
            ...
            <div className="bg-gray-50 rounded-lg p-2 border border-gray-300 h-[34px] flex items-center">
              <span className="text-[10px] text-gray-700 font-mono truncate">{guardrailsStatus || 'Waiting...'}</span>
            </div>
          </div>
        </div>
      </div>
