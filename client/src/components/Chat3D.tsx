            ...
            <div className="p-4 flex items-center gap-3" style={{ backgroundColor: "#2563EB" }}>
              ...
              <div>
                ...
                <p className="text-xs flex items-center gap-1" style={{ color: "#FCC700" }}>
                  <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: "#FCC700" }} /> Online
                </p>
              </div>
            </div>
                ...
                <AnimatePresence>
                  {!showEngagement ? (
                    ...
                    >
                      ...
                      <button onClick={() => setShowEngagement(true)} className="bg-primary text-white px-8 py-4 rounded-full font-bold shadow-lg hover:bg-primary/90 transition-all hover:scale-105 active:scale-95 text-sm z-10">
                        Continue
                      </button>
                    </motion.div>
                  ) : (
                      ...
                      {engagementMessages.map((msg, idx) => (
                        <AnimatePresence key={`engagement-${idx}`}>
                          {visibleMessages.includes(idx) && (
                            <motion.div 
                              className={`flex ${msg.type === 'sophie' ? 'justify-end' : 'justify-start'} py-2`}
