/**
 * State contract and audio-amplitude hooks powering SiriOrb.
 *
 * Vendored from SmoothUI's `ai-core` module by Eduardo Calvo, MIT licensed,
 * Copyright (c) 2024 Eduardo Calvo. See SiriOrb.tsx for why this is vendored
 * and what was changed (import path only).
 *
 * Note on `useAudioAmplitude`: upstream reads the MICROPHONE. A voice call
 * needs both sides, so the call page drives `amplitude` from its own analyser
 * over the remote WebRTC stream while she speaks. The RMS-to-0..1 maths here
 * is reused; only the source stream differs.
 */
import {
  type MotionValue,
  useAnimationFrame,
...
export const AI_STATE_MOTION: Record<AIState, AIStateMotion> = {
  ...
  // TUNED, not upstream. `streaming` is the state the receptionist spends most
  // of a call in, and upstream's values are calibrated for text streaming into
  // a chat box, where the orb is decoration beside the real content. Here it IS
  // the content: while she speaks it is the only thing moving on the page, so
  // reactivity goes to full (her voice drives the mesh exactly as the caller's
  // does when listening) and the rotation roughly doubles, so the colour inside
  // visibly churns while she talks rather than merely pulsing at the edge.
  // Saturation lifts a little for the same reason. Scale is left alone: a large
  // orb that grows while speaking pushes the layout around underneath it.
  streaming: {
    accent: null,
    glow: 0.55,
    hueRotate: -10,
