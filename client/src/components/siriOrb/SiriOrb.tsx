/**
 * Siri-style orb, shown while the AI receptionist is on a live call.
 *
 * Vendored from SmoothUI (https://smoothui.dev/docs/components/siri-orb) by
 * Eduardo Calvo, MIT licensed, Copyright (c) 2024 Eduardo Calvo. Vendored
 * rather than installed because SmoothUI publishes no npm package: the
 * shadcn registry copies the source into your tree, which is what this is.
 *
 * Two deliberate changes from upstream:
 *   - imports come from `framer-motion` instead of `motion/react`. They are
 *     the same library under two names, and framer-motion 12 is already a
 *     dependency here, so this avoids adding a second copy of it.
 *   - the Next.js "use client" directive is dropped: this is a Vite app.
 *
 * Everything else is upstream's, including the CSS engine (seven stacked
 * conic-gradients rotated through a registered @property) and the comments.
 */
// Upstream relies on the React UMD global for the `React.FC` type. This app
// compiles with the automatic JSX runtime and no UMD global access, so the
// namespace has to be imported explicitly.
