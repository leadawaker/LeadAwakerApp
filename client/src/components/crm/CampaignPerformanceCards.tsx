...
// data-as-labels: map raw campaign status values to i18n keys
const CAMPAIGN_STATUS_I18N_KEY: Record<string, string> = {
...
// data-as-labels: map raw campaign type values to i18n keys
const CAMPAIGN_TYPE_I18N_KEY: Record<string, string> = {
  "Cold Outreach": "campaignTypes.Cold Outreach",
  "Re-engagement": "campaignTypes.Re-engagement",
  "Follow-up": "campaignTypes.Follow-up",
  Event: "campaignTypes.Event",
};
  ...
  if (campaignsWithMetrics.length === 0) {
    return (
      <div data-testid="campaign-performance-cards">
        ...
        <DataEmptyState
          variant="campaigns"
          title={campaigns.length === 0 ? t("performance.noCampaigns") : t("performance.noData")}
          description={
            campaigns.length === 0
              ? t("performance.noCampaignsDescription")
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-1">
                    <span
                      ...
                      aria-hidden="true"
                    />
                    {campaign.type ? t(CAMPAIGN_TYPE_I18N_KEY[campaign.type] ?? `campaignTypes.${campaign.type}`, campaign.type) : t("performance.defaultType")}
                  </p>
                </div>
                <span
                ...
                >
                  {t(CAMPAIGN_STATUS_I18N_KEY[campaign.status] ?? `campaignStatus.${campaign.status}`, campaign.status)}
                </span>
              </div>
