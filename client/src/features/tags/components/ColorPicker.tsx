...
export function ColorPicker({ value, onChange, "data-testid": testId }: ColorPickerProps) {
  ...
  const fallbackLabel = value.charAt(0).toUpperCase() + value.slice(1);
  const label = t(`colors.${value}`, fallbackLabel);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("colorPicker.selectedColor", { name: label })}
        >
          <span
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {COLOR_PALETTE.map(([colorName, colorHex]) => (
            <button
              key={colorName}
              type="button"
              title={t(`colors.${colorName}`, colorName)}
              onClick={() => {
                onChange(colorName);
                setOpen(false);
              }}
