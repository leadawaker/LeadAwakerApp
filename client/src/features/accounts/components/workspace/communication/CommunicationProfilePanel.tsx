...
export function CommunicationProfilePanel({ accountId, fill = true }: { accountId: number; fill?: boolean }) {
  ...
  const { values: factValues, loading: factsLoading, saveFacts } = useOnboardingFacts(accountId);
  const [editing, setEditing] = useState(false);

  const completed = profile?.status === "completed";
  const showSummary = completed && !editing;

  const handleFinish = async (answers: ProfileAnswers, facts: FactValues) => {
  ...
  return (
    <Panel
      ...
      title={t("panel.title")}
      action={showSummary ? <EditButton label={t("summary.edit")} onClick={() => setEditing(true)} /> : undefined}
      style={fill ? { height: "100%" } : undefined}
    >
      {loading || factsLoading ? (
        <div style={{ padding: "40px 0", textAlign: "center", color: "var(--mute-2)", fontSize: 13 }}>…</div>
      ) : showSummary ? (
        <ProfileSummary answers={profile!.answers} facts={factValues} />
      ) : (
        <ProfileWizard
