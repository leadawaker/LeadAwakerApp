...
export function KBPanel({ accountId }: { accountId: number }) {
  ...
  return (
    <Panel icon={<BookOpen size={16} />} title={t("panels.knowledgeBase")} count={t("knowledge.countEntries", { count: entries.length })}
      action={!formOpen ? <PanelAction wine icon={<Plus size={12} />} onClick={() => openAdd()}>{t("knowledge.add")}</PanelAction> : undefined}
      style={{ height: "100%" }}>
      {formOpen && (
