...
export function AccountDetailsDialog({
  ...
  async function handleStatusToggle(checked: boolean) {
    ...
    try {
      ...
      toast({
        title: newStatus === "Active" ? t("detail.activated") : t("detail.deactivated"),
        description: `${account?.name || t("detail.unnamedAccount")} ${t("detail.isNow")} ${newStatus.toLowerCase()}.`,
      });
    } catch {
      toast({
        variant: "destructive",
        title: t("detail.statusUpdateFailed"),
        description: t("detail.statusUpdateFailedDesc"),
      });
    } finally {
      setToggling(false);
    }
    ...
    return (
      <div className="space-y-1">
        ...
        <p className="text-[11px] text-muted-foreground">
          {t("fields.serviceCategoriesHint")}
        </p>
      </div>
    );
  }
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          >
            {badge.icon}
            {t(STATUS_I18N_KEY[status] ?? "status.unknown")}
          </Badge>
          {account.type && (
            <Badge
              variant="outline"
            ...
            >
              {t(TYPE_I18N_KEY[account.type] ?? account.type)}
            </Badge>
          )}
          ...
          <div
            ...
            title={status === "Active" ? t("detail.clickToDeactivate") : t("detail.clickToActivate")}
          >
            <Switch
  ...
  const bodyContent = (
    <div className="space-y-0 pb-2">
      <SectionHeader icon={<Building2 className="h-4 w-4" />} title={t("sections.basicInfo")} />
      <FieldRow label={t("fields.name")}>{textField("name", "Account name")}</FieldRow>
      <FieldRow label={t("fields.status")}>{selectField("status", STATUS_OPTIONS, (o) => t(STATUS_I18N_KEY[o] ?? o))}</FieldRow>
      <FieldRow label={t("fields.type")}>{selectField("type", TYPE_OPTIONS, (o) => t(TYPE_I18N_KEY[o] ?? o))}</FieldRow>
      <FieldRow label={t("fields.businessNiche")}>
      ...
      <FieldRow label={t("fields.phone")}>{textField("phone", "+1 555 000 0000")}</FieldRow>
      <FieldRow label={t("fields.website")}>{textField("website", "https://...")}</FieldRow>

      <SectionHeader icon={<Clock className="h-4 w-4" />} title={t("sections.scheduleTimezone")} />
      <FieldRow label={t("fields.timezone")}>{selectField("timezone", TIMEZONE_OPTIONS)}</FieldRow>
      <FieldRow label={t("fields.businessHoursOpen")}>{timeField("business_hours_start", "09:00")}</FieldRow>
      <FieldRow label={t("fields.businessHoursClose")}>{timeField("business_hours_end", "17:00")}</FieldRow>
      <FieldRow label={t("fields.maxDailySends")}>{numberField("max_daily_sends", "500")}</FieldRow>

      <SectionHeader icon={<Bot className="h-4 w-4" />} title={t("sections.aiMessaging")} />
      <FieldRow label={t("fields.aiName")}>{textField("default_ai_name", "e.g. Alex")}</FieldRow>
      <FieldRow label={t("fields.aiRole")}>{textField("default_ai_role", "e.g. admin support")}</FieldRow>
      <FieldRow label={t("fields.aiStyle")}>{textField("default_ai_style", "e.g. Natural, human, low pressure")}</FieldRow>
      <FieldRow label={t("fields.typoFrequency")}>{selectField("default_typo_frequency", TYPO_FREQUENCY_OPTIONS, (o) => t(TYPO_FREQ_I18N_KEY[o] ?? o))}</FieldRow>
      <FieldRow label={t("fields.optOutKeyword")}>{textField("opt_out_keyword", "e.g. STOP")}</FieldRow>
      <FieldRow label={t("fields.preferredTerminology")}>{textField("preferred_terminology", "")}</FieldRow>
      <FieldRow label={t("fields.disclosureText")}>{textField("data_collection_disclosure", "", "textarea")}</FieldRow>

      <SectionHeader icon={<FileText className="h-4 w-4" />} title={t("sections.descriptionNotes")} />
      <FieldRow label={t("fields.businessDescription")}>
        <div data-testid="field-business_description">
          {textField("business_description", "Describe the business...", "textarea")}
        </div>
      </FieldRow>
      <FieldRow label={t("fields.serviceCategories")}>{serviceCategoriesField()}</FieldRow>
      <FieldRow label={t("sections.notes")}>{textField("notes", "Internal notes...", "textarea")}</FieldRow>

      <SectionHeader icon={<Globe className="h-4 w-4" />} title={t("sections.twilioIntegration")} />
      <FieldRow label={t("fields.accountSid")}>{textField("twilio_account_sid", "ACxxxxxxxx")}</FieldRow>
      <FieldRow label={t("fields.authToken")}>{textField("twilio_auth_token", "")}</FieldRow>
      <FieldRow label={t("fields.messagingServiceSid")}>{textField("twilio_messaging_service_sid", "MGxxxxxxxx")}</FieldRow>
      <FieldRow label={t("fields.fromNumber")}>{textField("twilio_default_from_number", "+1 555 000 0000")}</FieldRow>
      <FieldRow label={t("fields.apiKeyIntake")}>{textField("webhook_secret", "")}</FieldRow>
      <FieldRow label={t("fields.intakeUrl")}>
        <span className="text-[11px] font-mono text-foreground/70 select-all">https://webhooks.leadawaker.com/api/leads/intake</span>
      </FieldRow>

      <SectionHeader icon={<Phone className="h-4 w-4" />} title={t("sections.instagramIntegration")} />
      <FieldRow label={t("fields.instagramUserId")}>{textField("instagram_user_id", "IG Business Account User ID")}</FieldRow>
      <FieldRow label={t("fields.accessToken")}>{textField("instagram_access_token", "Long-lived Page access token")}</FieldRow>
    </div>
  );
