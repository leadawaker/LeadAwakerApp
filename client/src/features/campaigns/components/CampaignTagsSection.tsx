...
import type { Tag as TagType, TagTableItem, EnrichedTag, TagSortOption, TagAutoAppliedFilter } from "@/features/tags/types";
...
interface CampaignTagsSectionProps {
  ...
  /** Ref the parent uses to read the current campaign tags (for Save as Template) */
  tagsDataRef?: MutableRefObject<TagType[] | null>;
...
export function CampaignTagsSection({
  ...
  useEffect(() => {
    createRef.current = async (data) => { await handleCreate(data); };
  }, [handleCreate, createRef]);

  /* ── Expose tags to parent via ref (for Save as Template) ───────────── */
  useEffect(() => {
    if (tagsDataRef) tagsDataRef.current = tags;
  }, [tags, tagsDataRef]);
