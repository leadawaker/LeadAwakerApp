...
export function PerformancePanel({ metrics, animTrigger }: {
  ...
  return (
    <PanelShell variant="inset" testId="campaign-detail-view-metrics">
      <SectionHead
      ...
      {/* Metrics (4-across, square) + trend chart on the right; wraps on narrow screens */}
      <div className="bold-metrics-row">
        <div
          className="perf-metrics grid gap-3"
          style={{ gridTemplateColumns: "repeat(4, minmax(0, 1fr))", flex: "1 1 56%", minWidth: 360 }}
        >
        ...
        <div className="perf-chart flex flex-col" style={{ flex: "1 1 40%", minWidth: 280 }}>
          <div className="flex items-center justify-between mb-2">
            <span className="eyebrow eyebrow-sm">{t("summary.performanceTrends")}</span>
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1 text-[10px]" style={{ color: "var(--mute)" }}>
                <span className="w-3 h-[2px] rounded-full inline-block" style={{ background: "var(--stage-responded)" }} />
                {t("summary.responsePercent")}
              </span>
              <span className="flex items-center gap-1 text-[10px]" style={{ color: "var(--mute)" }}>
                <span className="w-3 h-[2px] rounded-full inline-block" style={{ background: "var(--wine)" }} />
                {t("summary.bookingPercent")}
              </span>
            </div>
          </div>
          <div className="flex-1 min-h-[180px]" key={trig} data-testid="campaign-detail-view-trends">
            <ResponsiveContainer width="100%" height="100%">
              ...
              <defs>
                <linearGradient id="fillResp" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#3F8E8E" stopOpacity={0.18} />
                  <stop offset="100%" stopColor="#3F8E8E" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey={t("summary.responsePercent")} stroke="#3F8E8E" strokeWidth={2} fill="url(#fillResp)" dot={false} activeDot={{ r: 3 }} />
              <Area type="monotone" dataKey={t("summary.bookingPercent")} stroke="var(--wine)" strokeWidth={2} fill="none" dot={false} activeDot={{ r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
          </div>
        </div>
