...
export function useCampaignsData(accountId?: number) {
    ...
    try {
      ...
      const normalized = campaignsList.map((c: any) => {
        ...
        return {
          ...
          account_whatsapp_sender_sid: (account as any)?.whatsapp_sender_sid || null,
          ...
          account_whatsapp_max_daily_sends_is_manual: (account as any)?.whatsapp_max_daily_sends_is_manual ?? false,
  ...
  // Auto-refresh on SSE campaigns_changed events
  useEffect(() => {
