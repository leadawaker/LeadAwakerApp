...
export function InvoiceCreatePanel({
  ...
  function validate(): Record<string, string> {
    ...
    if (!title.trim()) {
      errs.title = t("invoices.form.titleRequired");
    }
    ...
    if (!hasValidItem) {
      errs.lineItems = t("invoices.form.addLineItemError");
    }

    if (issuedDate && dueDate && dueDate < issuedDate) {
      errs.dueDate = t("invoices.form.dueDateError");
    }
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    try {
      if (isEditMode && editingInvoice) {
        ...
        toast({ title: t("invoices.form.invoiceUpdated"), description: t("invoices.form.invoiceUpdatedDesc", { title: title || t("invoices.form.newInvoice") }) });
      } else {
        await onCreate(payload);
        toast({ title: t("invoices.form.invoiceCreated"), description: t("invoices.form.invoiceCreatedDesc", { title: title || t("invoices.form.newInvoice") }) });
      }
      onClose();
    } catch (err) {
      const message = err instanceof Error ? err.message : t("invoices.form.somethingWentWrong");
      toast({ title: t("invoices.form.failedToSave"), description: message, variant: "destructive" });
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex items-start justify-between px-5 pt-6 pb-4 border-b border-border/30 shrink-0">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {isEditMode ? t("invoices.form.updateInvoiceDetails") : t("invoices.form.fillInDetails")}
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="px-5 py-4 space-y-5">
          ...
          <div className={cn(
            ...
            isAgencyUser && accountId
              ? "grid-cols-1 md:grid-cols-[1fr_1fr_1fr_7rem]"
              : isAgencyUser
                ? "grid-cols-1 md:grid-cols-[1fr_1fr_7rem]"
                : "grid-cols-1 md:grid-cols-[1fr_7rem]"
          )}>
            <div className="space-y-1.5">
              <Label htmlFor="cp-title" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.title")}</Label>
              <Input
                id="cp-title"
                placeholder={t("invoices.form.titlePlaceholder")}
                value={title}
                onChange={(e) => {
            ...
            {isAgencyUser && (
              <div className="space-y-1.5">
                <Label htmlFor="cp-account" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.account")}</Label>
                <Select value={accountId} onValueChange={setAccountId}>
                  <SelectTrigger id="cp-account">
                    <SelectValue placeholder={t("invoices.form.selectAccount")} />
                  </SelectTrigger>
                  <SelectContent>
                    {accounts.map((a) => (
              ...
              <div className="space-y-1.5">
                ...
                >
                  <SelectTrigger id="cp-campaign" disabled={campaignsLoading}>
                    <SelectValue placeholder={campaignsLoading ? t("invoices.form.loading") : t("invoices.form.optional")} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="cp-inv-num" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.invoiceNumber")}</Label>
              <Input
                id="cp-inv-num"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="cp-currency" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.currency")}</Label>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="cp-issued" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.issued")}</Label>
              <Input
                id="cp-issued"
                type="date"
                value={issuedDate}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <Label htmlFor="cp-due" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.dueDate")}</Label>
                ...
                <div className="flex items-center gap-0.5 bg-muted rounded-full p-0.5">
                  {DUE_PRESETS.map((days) => (
                    <button
                      ...
                      title={t("invoices.form.dueInDays", { count: days })}
                      className={cn(
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.lineItems")}</Label>
              {errors.lineItems && (
            ...
            {/* Header — desktop only */}
            <div className="hidden md:grid grid-cols-[1fr_3.5rem_5.5rem_5.5rem_1.5rem] gap-2 text-[10px] font-semibold text-muted-foreground/60 uppercase tracking-wide px-0.5">
              <span>{t("invoices.form.description")}</span>
              <span>{t("invoices.form.qty")}</span>
              <span>{t("invoices.form.unitPrice")}</span>
              <span>{t("invoices.form.amount")}</span>
              <span />
            </div>
            ...
            {lineItems.map((item, idx) => (
              <div key={idx}>
                {/* Mobile card layout */}
                <div className="md:hidden rounded-lg border border-border/40 bg-muted/20 p-2.5 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="h-9 w-9 rounded flex items-center justify-center text-muted-foreground hover:text-destructive disabled:opacity-30 shrink-0"
                    >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-semibold text-muted-foreground/60 uppercase tracking-wide">{t("invoices.form.qty")}</span>
                      <Input
                        type="number"
                        min={0}
                        step={0.01}
                        ...
                        className="h-9 text-center tabular-nums text-[13px]"
                      />
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-semibold text-muted-foreground/60 uppercase tracking-wide">{t("invoices.form.unitPrice")}</span>
                      <Input
                        type="number"
                        min={0}
                        step={0.01}
                        ...
                        className="h-9 tabular-nums text-[13px]"
                      />
                    </div>
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-semibold text-muted-foreground/60 uppercase tracking-wide">{t("invoices.form.amount")}</span>
                      <Input
                        readOnly
                        tabIndex={-1}
                        value={item.amount.toFixed(2)}
                        className="h-9 tabular-nums bg-muted/40 cursor-default text-[13px]"
                      />
                    </div>
                  </div>
                </div>
                {/* Desktop row layout */}
                <div className="hidden md:grid grid-cols-[1fr_3.5rem_5.5rem_5.5rem_1.5rem] gap-2 items-center">
                  <Input
                    placeholder={t("invoices.form.descriptionPlaceholder")}
                    value={item.description}
                    onChange={(e) => updateLineItem(idx, "description", e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="cp-tax" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.taxPercent")}</Label>
                <Input
                  id="cp-tax"
                  type="number"
                  min={0}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="cp-discount" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.discount")}</Label>
                <Input
                  id="cp-discount"
                  type="number"
                  min={0}
            ...
            <div className="flex flex-col justify-end space-y-1 text-sm bg-muted/50 rounded-lg p-3">
              ...
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">{t("invoices.detail.taxWithPercent", { percent: taxPercent })}</span>
          ...
          >
            ...
            />
            {showMoreFields ? t("invoices.form.fewerFields") : t("invoices.form.moreFields")}
          </button>
          ...
          {showMoreFields && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="cp-sent-at" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.sentDate")}</Label>
                <Input
                  id="cp-sent-at"
                  type="date"
                  value={sentAt}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="cp-paid-at" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.paidDate")}</Label>
                <Input
                  id="cp-paid-at"
                  type="date"
                  value={paidAt}
          ...
          {/* ── Notes ────────────────────────────────────────────────────── */}
          <div className="space-y-1.5">
            <Label htmlFor="cp-notes" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.notes")}</Label>
            <Textarea
              id="cp-notes"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="cp-payment" className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("invoices.form.paymentInfo")}</Label>
              ...
              <div className="flex items-center gap-1 bg-muted rounded-full p-0.5">
                {(Object.entries(SENDER_PRESETS) as [SenderPreset, typeof SENDER_PRESETS[SenderPreset]][]).map(([key, preset]) => (
                  <button
                    ...
                    title={t("invoices.form.useBankDetails", { label: preset.label })}
                    className={cn(
            ...
            <Textarea
              id="cp-payment"
              placeholder={t("invoices.form.paymentInstructionsPlaceholder")}
              rows={6}
              value={paymentInfo}
