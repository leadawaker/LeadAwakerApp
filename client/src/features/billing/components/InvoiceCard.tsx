        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-1">
            <span className="text-[16px] font-semibold font-heading text-foreground truncate">
              {invoice.title ? toDisplayTitle(invoice.title, t("invoices.card.untitledInvoice")) : (invoice.invoice_number || t("invoices.card.untitled"))}
            </span>
          ...
          <div className="flex items-center justify-between gap-1 mt-0.5">
            <span className="text-[11px] text-muted-foreground truncate">
              {invoice.account_name || t("invoices.card.noAccount")}
            </span>
            <span
            ...
            {dueDate && (
              <span className={cn(
                ...
                {displayStatus === "Overdue"
                  ? t("invoices.card.dOverdue", { count: Math.abs(daysUntilDue!) })
                  : displayStatus === "Paid"
                    ? t("invoices.card.paid")
                    : daysUntilDue !== null && daysUntilDue <= 7
                      ? t("invoices.card.dueIn", { count: daysUntilDue })
                      : t("invoices.card.due", { date: dueDate.toLocaleDateString("en-US", { month: "short", day: "numeric" }) })
                }
              </span>
            )}
          </div>
