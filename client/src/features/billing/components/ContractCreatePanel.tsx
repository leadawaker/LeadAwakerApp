...
const DEAL_TYPE_OPTIONS = [
  { value: "performance",      tKey: "contracts.form.dealTypes.performance",      subTKey: "contracts.form.dealTypes.performanceSub" },
  { value: "cost_passthrough", tKey: "contracts.form.dealTypes.costPassthrough",  subTKey: "contracts.form.dealTypes.costPassthroughSub" },
  { value: "fixed_fee",        tKey: "contracts.form.dealTypes.fixedFee",         subTKey: "contracts.form.dealTypes.fixedFeeSub" },
  { value: "deposit",          tKey: "contracts.form.dealTypes.deposit",          subTKey: "contracts.form.dealTypes.depositSub" },
  { value: "monthly_retainer", tKey: "contracts.form.dealTypes.monthlyRetainer",  subTKey: "contracts.form.dealTypes.monthlyRetainerSub" },
  { value: "hybrid",           tKey: "contracts.form.dealTypes.hybrid",           subTKey: "contracts.form.dealTypes.hybridSub" },
];

const PAYMENT_TRIGGER_OPTIONS = [
  { value: "call_booked",  tKey: "contracts.form.paymentTriggers.callBooked" },
  { value: "closed_sale",  tKey: "contracts.form.paymentTriggers.closedSale" },
];

const CURRENCY_OPTIONS = ["EUR", "USD", "GBP", "BRL"] as const;

const END_DATE_PRESETS = [
  { tKey: "contracts.endDatePresets.1mo",    months: 1  },
  { tKey: "contracts.endDatePresets.3mo",    months: 3  },
  { tKey: "contracts.endDatePresets.6mo",    months: 6  },
  { tKey: "contracts.endDatePresets.1yr",    months: 12 },
  { tKey: "contracts.endDatePresets.custom", months: 0  },
];

const CADENCE_OPTIONS = [
  { value: "weekly",   tKey: "contracts.form.cadenceOptions.weekly" },
  { value: "biweekly", tKey: "contracts.form.cadenceOptions.biweekly" },
  { value: "monthly",  tKey: "contracts.form.cadenceOptions.monthly" },
];
...
export function ContractCreatePanel({
  ...
  const handleCreateCampaign = async () => {
    ...
    try {
      ...
      toast({ title: t("contracts.form.campaignCreated"), description: t("contracts.form.campaignCreatedDesc", { name: newCamp.name }) });
    } catch {
      toast({ title: t("contracts.form.failedToCreateCampaign"), variant: "destructive" });
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    if (!title.trim()) {
      toast({ title: t("contracts.form.titleRequired"), variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      ...
      toast({ title: t("contracts.form.contractCreated"), description: t("contracts.form.contractCreatedDesc", { title }) });
      onClose();
    } catch (err) {
      const msg = err instanceof Error ? err.message : t("contracts.form.somethingWentWrong");
      toast({ title: t("contracts.form.failedToCreateContract"), description: msg, variant: "destructive" });
    } finally {
      setSaving(false);
    }
  }, [
  ...
  return (
    <div className={cn("flex flex-col h-full overflow-hidden", compact && "")} data-testid="contract-create-panel">
      ...
      {/* Two-column body OR stacked if compact — also stacked on mobile */}
      <div className={cn("flex-1 min-h-0 flex overflow-hidden", compact ? "flex-col" : "flex-col md:flex-row")}>
        ...
        <div className={cn(
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.title")}</Label>
              <Input
            ...
            {isAgencyUser && (
              <div className="space-y-1.5">
                <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.account")}</Label>
                <Select value={accountId || "__none__"} onValueChange={v => setAccountId(v === "__none__" ? "" : v)}>
                  <SelectTrigger><SelectValue placeholder={t("contracts.form.selectAccount")} /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__none__">{t("contracts.form.noAccount")}</SelectItem>
                    {accounts.map(a => (
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                {t("contracts.form.signerName")} <span className="font-normal normal-case text-muted-foreground/50">{t("contracts.form.signerNameHint")}</span>
              </Label>
              <Input
                placeholder={t("contracts.form.signerNamePlaceholder")}
                value={signerName}
            ...
            {isAgencyUser && accountId && (
              <div className="space-y-1.5">
                <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.campaign")}</Label>
                <div className="flex gap-1.5">
                  <Select value={campaignId || "__none__"} onValueChange={v => setCampaignId(v === "__none__" ? "" : v)}>
                    <SelectTrigger className="flex-1" disabled={campaignsLoading}>
                      <SelectValue placeholder={campaignsLoading ? t("invoices.form.loading") : t("contracts.form.selectCampaign")} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <button
                    ...
                    title={t("contracts.form.createNewCampaign")}
                ...
                {showNewCampaignForm && (
                  <div className="flex gap-1.5 mt-1">
                    <Input
                      placeholder={t("contracts.form.newCampaignName")}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.timezone")}</Label>
                <Input
                  value={timezone}
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 font-heading">{t("contracts.form.dealStructure")}</p>

            {/* Deal Type pills */}
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.dealType")}</Label>
              <div className="grid grid-cols-2 gap-1.5">
                {DEAL_TYPE_OPTIONS.map(opt => (
                  <button key={opt.value} type="button"
                    ...
                    <span className="text-[9px] text-foreground/50 leading-snug">{t(opt.subTKey)}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.valuePerBooking")}</Label>
              <div className="flex gap-1.5">
            ...
            {(showCostPassthrough || showFixedFee || showDeposit || showMonthlyFee) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {showCostPassthrough && (
                  <div className="space-y-1.5">
                    <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.costPassthroughPercent")}</Label>
                ...
                {showDeposit && (
                  <div className="space-y-1.5">
                    <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.deposit")}</Label>
                    <Input
                      type="number" min={0} step={0.01} placeholder="0.00"
                      value={depositAmount}
                ...
                {showMonthlyFee && (
                  <div className="space-y-1.5">
                    <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.monthlyFee")}</Label>
                    <Input
                      type="number" min={0} step={0.01} placeholder="0.00"
                      value={monthlyFee}
            ...
            <div className="h-px bg-border/40" />
            <p className="text-[10px] font-bold uppercase tracking-widest text-foreground/40 font-heading">{t("contracts.form.invoicingAndPayment")}</p>

            {/* Invoice Cadence */}
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.invoiceCadence")}</Label>
              <div className="flex gap-1.5">
            ...
            {/* Payment Preset */}
            <div className="space-y-1.5">
              <Label className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{t("contracts.form.paymentAccount")}</Label>
              <div className="flex gap-1.5">
        ...
        {/* ── RIGHT: Contract text column — hidden on mobile to keep form accessible ── */}
        <div className={cn("flex-col bg-muted/30", compact ? "flex flex-none h-[340px]" : "hidden md:flex flex-1 min-w-0")}>

          {/* Right header */}
          <div className="flex items-center justify-between px-4 pt-3 pb-2 border-b border-border/20 shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold uppercase tracking-widest text-foreground/40 font-heading">{t("contracts.form.contractText")}</span>
              {isManuallyEdited && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 text-[10px] font-semibold">
                  ...
                  {t("contracts.detail.manuallyEdited")}
                </span>
              )}
            </div>
            <div className="flex items-center gap-1">
              ...
              >
                ...
                {copied ? t("contracts.form.copied") : t("contracts.form.copy")}
              </button>
            </div>
          </div>
          ...
          <div className="px-4 py-1.5 border-t border-border/20 shrink-0">
            <span className="text-[10px] text-muted-foreground/50 tabular-nums">
              {t("contracts.detail.characters", { count: editedText.length })}
            </span>
          </div>
