...
import loginHero1 from "../assets/login-hero.webp";
import loginHero2 from "../assets/login-hero-2.webp";

const HERO_IMAGES = [loginHero1, loginHero2] as const;

export default function Login() {
  const { t } = useTranslation("login");
  const [heroSrc] = useState(() => {
    const prev = Number(localStorage.getItem("app_hero_idx") || "0");
    const next = (prev + 1) % HERO_IMAGES.length;
    localStorage.setItem("app_hero_idx", String(next));
    return HERO_IMAGES[next];
  });
  const isLady = heroSrc === loginHero1;
  ...
  useEffect(() => {
    const isAuthed = Boolean(localStorage.getItem("app_auth"));
    const remembered = localStorage.getItem("app_remember_me") === "true";
    if (isAuthed && remembered) {
      // TODO: update this redirect to your app's home route
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // TODO: update localStorage key names for your app
      localStorage.setItem("app_auth", "session");
      localStorage.setItem("app_user_email", user.email ?? email);
      localStorage.setItem("app_user_role", user.role ?? "Viewer");
      localStorage.setItem("app_user_name", user.fullName1 ?? user.email ?? email);
      if (user.avatarUrl) {
        localStorage.setItem("app_user_avatar", user.avatarUrl);
      }
      localStorage.setItem("app_current_account_id", String(user.accountsId ?? 1));
      ...
      // TODO: update this redirect to your app's home route after login
      setLocation("/dashboard");
    } catch {
  ...
  return (
    <div className="legacy-app min-h-screen flex items-center bg-slate-50 dark:bg-background py-12">
