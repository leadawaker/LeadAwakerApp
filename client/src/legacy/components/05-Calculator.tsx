...
import { useCalculator } from "./05-calculator/useCalculator";
import { CalculatorSliders } from "./05-calculator/CalculatorSliders";
import { CalculatorAdvanced } from "./05-calculator/CalculatorAdvanced";
import { CalculatorHero } from "./05-calculator/CalculatorHero";
import { CalculatorChart } from "./05-calculator/CalculatorChart";

// TODO: replace with your WhatsApp number, e.g. "31612345678"
...
export default function Calculator() {
  ...
  const accentColor = calc.selectedScenario === 0 ? "var(--brand-indigo-muted)" : calc.selectedScenario === 2 ? "var(--brand-yellow)" : "var(--brand-indigo)";
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <button
            ...
            className="absolute -top-5 right-0 inline-flex items-center justify-center w-10 h-10 rounded-full border border-border bg-white dark:bg-background text-muted-foreground hover:text-[var(--brand-indigo)] hover:border-[var(--brand-indigo)] transition-colors"
          >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-center mb-8">
            <div role="tablist" className="inline-flex bg-[var(--bg-secondary)] dark:bg-[#1e2535] border border-[#e5e7eb] dark:border-white/10 rounded-full p-1">
              {([
                ...
                return (
                  <button
                    ...
                    style={{ backgroundColor: active ? "var(--brand-indigo-active)" : "transparent", color: active ? "#fff" : "var(--brand-indigo-muted)" }}
                  >
                    {label}
                  </button>
                );
          ...
          >
            ...
            <a
              href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(t(calc.profitMode ? "calculator.cta.waMessageProfit" : "calculator.cta.waMessage", { leads: calc.leads.toLocaleString(), dealValue: calc.formatCurrency(calc.dealValue), revenue: calc.formatCurrency(calc.selectedRevenue) }))}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-[var(--brand-indigo)] text-white font-heading font-bold text-base hover:bg-[var(--brand-indigo-hover)] transition-colors"
            >
