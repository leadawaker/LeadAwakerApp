...
import { sliderPct } from "./useCalculator";
...
export function CalculatorSliders({
  ...
  return (
    <motion.div layout className="flex flex-col gap-4 w-full md:w-[368px] shrink-0">
      ...
      <motion.div
        ...
        className="bg-[var(--bg-secondary)] dark:bg-[#1e2535] rounded-2xl p-8"
      >
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-[var(--brand-indigo)]/10 flex items-center justify-center shrink-0">
            <Users className="w-5 h-5 text-[var(--brand-indigo)]" />
          </div>
        ...
        {editingLeads ? (
          <input
            ...
            className="text-4xl font-bold font-heading text-[var(--brand-indigo)] mb-4 w-full bg-transparent outline-none"
          />
        ) : (
          <div
            className="text-4xl font-bold font-heading text-[var(--brand-indigo)] mb-4 cursor-text hover:opacity-70 transition-opacity"
        ...
        <input
          ...
          style={{ background: `linear-gradient(to right, var(--brand-indigo) ${sliderPct(Math.min(Math.max(leads, 500), 50000), 500, 50000)}%, rgba(100,116,139,0.3) ${sliderPct(Math.min(Math.max(leads, 500), 50000), 500, 50000)}%)` }}
        />
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-[var(--brand-yellow)]/10 flex items-center justify-center shrink-0">
            <DollarSign className="w-5 h-5 text-[var(--brand-yellow)]" />
          </div>
        ...
        {editingDeal ? (
          <input
            ...
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === "Escape") (e.target as HTMLInputElement).blur(); }}
            className="text-4xl font-bold font-heading text-[var(--brand-yellow)] mb-4 w-full bg-transparent outline-none"
          />
        ) : (
          <div
            className="text-4xl font-bold font-heading text-[var(--brand-yellow)] mb-4 cursor-text hover:opacity-70 transition-opacity"
        ...
        <input
          ...
          className="w-full h-2 rounded-full appearance-none cursor-pointer"
          style={{ background: `linear-gradient(to right, var(--brand-yellow) ${sliderPct(Math.min(Math.max(dealValue, dealConfig.min), dealConfig.max), dealConfig.min, dealConfig.max)}%, rgba(100,116,139,0.3) ${sliderPct(Math.min(Math.max(dealValue, dealConfig.min), dealConfig.max), dealConfig.min, dealConfig.max)}%)` }}
        />
        <div className="flex justify-between text-sm text-muted-foreground mt-2">
