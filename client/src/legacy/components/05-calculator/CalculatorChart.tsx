...
import { ScenarioResult, HoveredDot } from "./useCalculator";
  ...
  return (
    <div className="mt-4">
      ...
      <svg viewBox={`0 0 ${viewBoxW} ${viewBoxH}`} className="w-full h-44" style={{ overflow: "visible" }} onMouseLeave={() => setHoveredDot(null)}>
        ...
        <polyline fill="none" stroke="var(--brand-indigo)" strokeWidth="2.5" points={pointsAttr} />
        {breakevenReachable && (() => {
          const bx = xFor(breakevenDeal);
          return (
            <g>
              <line x1={bx} x2={bx} y1={spendY} y2={padT + plotH} stroke="var(--brand-yellow)" strokeWidth="1.5" strokeDasharray="2 3" />
              <circle cx={bx} cy={spendY} r="5" fill="var(--brand-yellow)" />
              <text x={bx} y={padT + plotH + 14} fontSize="10" fill="var(--brand-yellow)" textAnchor="middle" fontWeight="bold">
        ...
        {closed > 0 && (
          <g>
            <circle cx={xFor(closed)} cy={yFor(totalRevenue)} r="4" fill="var(--brand-indigo)" />
            <text x={xFor(closed) - 6} y={yFor(totalRevenue) - 6} fontSize="10" fill="var(--brand-indigo)" textAnchor="end" fontWeight="bold">
        ...
        {closed > 0 && [0.25, 0.5, 0.75, 1].map((frac) => {
          ...
          return (
            <g key={frac}>
              ...
              <circle cx={cx} cy={cy} r={isHovered ? 5 : 3.5} fill="var(--brand-indigo)" opacity={isHovered ? 1 : 0.5}
