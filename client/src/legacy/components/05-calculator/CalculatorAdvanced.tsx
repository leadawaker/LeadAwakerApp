...
import { Scenario, sliderPct } from "./useCalculator";
...
function InfoTip({ text }: { text: string }) {
  return (
    <span className="relative inline-flex items-center group">
      <Info className="w-3.5 h-3.5 text-muted-foreground group-hover:text-[var(--brand-indigo)] cursor-help" />
...
export function CalculatorAdvanced({
  ...
  return (
    <motion.div
      ...
      className="relative bg-[var(--bg-secondary)] dark:bg-[#1e2535] rounded-2xl p-6 w-full md:w-[380px] shrink-0 flex flex-col gap-[26px]"
      style={{ zIndex: 20 }}
    >
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        >
          {scenarios.map((sc, i) => {
            ...
            const btnColor = i === 0 ? "var(--brand-indigo-muted)" : i === 2 ? "var(--brand-yellow)" : "var(--brand-indigo)";
            return (
              <button
                key={i}
                type="button"
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="relative inline-block">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border bg-white dark:bg-background text-sm font-medium text-muted-foreground hover:text-foreground hover:border-[var(--brand-indigo)] transition-colors"
          >
            ...
            <div className="absolute right-0 mt-1 w-36 rounded-xl border border-border bg-white dark:bg-background shadow-lg py-1" style={{ zIndex: 50 }}>
              {Object.values(CURRENCIES).map((c) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm transition-colors hover:bg-[var(--brand-indigo)]/10 ${c.code === currencyCode ? "text-[var(--brand-indigo)] font-semibold" : "text-foreground"}`}
                >
                  {c.symbol} {c.code}
                </button>
              ))}
